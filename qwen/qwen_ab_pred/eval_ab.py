...
def read_pred_jsonl(p_path: Path) -> Dict[str, Set[str]]:
    """
    JSONLの1行=1JSONを基本としつつ、
    たまにある `}{` 連結（改行なし）も自動で補正して読む。
    ...
    for lab in root.findall(".//labels/label"):
        ...
        for attr in lab.findall("./attributes/attribute"):
            ...
            if _norm(aname) == "subclass":
                values_node = attr.find("./values")
                if values_node is not None:
                    # ① <values><value>…</value></values> の場合
                    child_vals = [v.text for v in values_node.findall("./value") if v.text]
                    if child_vals:
                        subclass_val = child_vals[0].strip()
                    else:
                        # ② <values>comma,separated</values> や単一文字列の場合
                        subclass_val = (values_node.text or "").strip()
                break
